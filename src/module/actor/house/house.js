 ...
 * @category Actor
 */
export class House extends ActorChronicle {
    roleMap = {
        HEAD: "head",
        STEWARD: "steward",
        HEIR: "heirs",
    ...
    characterHasRole(actorId, role) {
        ...
        switch (role) {
            case "STEWARD":
            case "HEAD":
                if (this.getData().members[this.roleMap[role]].id === actorId) {
                    result.hasRole = true;
                }
                break;
            case "HEIR":
    ...
    _getMemberIndexIfExists(role, id, list = undefined) {
        if (!list)
            list = this.getData().members[this.roleMap[role]];
    ...
    _removeCharacterFromRole(role, actorId, index = -1) {
        ...
        switch (role) {
            ...
            case "RETAINER":
            case "SERVANT":
                let list = this.getData().members[this.roleMap[role]];
                if (index < 0)
    ...
    getCharactersFromRole(role) {
        ...
        let membersData = this.getData().members[role];
        let members = [];
